...
NEGATED_EXPLOITATION_PATTERN = re.compile(
    ...
    r"(?:has|have|had)\s+never\s+been\s+"
    r"(?:(?:actively|currently)\s+)*(?=exploit|in the wild|weaponiz|under attack)|"
    r"(?:is|are|was|were)\s+never\s+"
    r"(?:(?:actively|currently)\s+)*(?:being\s+)?"
    r"(?=exploit|in the wild|weaponiz|under attack)|"
    ...
    r"(?=exploit|weaponiz|come under attack)|"
