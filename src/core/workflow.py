...
async def generate_report(state: ExploitationAnalysisState) -> ExploitationAnalysisState:
    ...
    try:
        ...
        # Campaigns mapping
        campaigns_map = build_campaigns_mapping(report, vt_api_key)
        write_campaigns_json(campaigns_map, out_dir)
        ...
        if docs_dir.exists():
            ...
            write_campaigns_json(campaigns_map, docs_dir)
        logger.info("Wrote actors.json, malware.json, campaigns.json next to report and docs/")
    except Exception as e:
