...
def extract_exploitation_details(text: str) -> Dict[str, Any]:
    ...
    result = {"exploitation_status": "", "affected_systems": [], "attack_vectors": []}
...
def extract_cvss_scores(text: str) -> List[Dict[str, Any]]:
    ...
    cvss_patterns = [
        ...
        re.compile(
            r"CVSS\s+(?:v?(\d+\.\d+))?\s+(?:score:?|of|rating:?|base:?)\s+(\d+\.\d+)",
            re.IGNORECASE,
        ),
        # Alternative pattern: "scored 9.8 on the CVSS scale"
        re.compile(
            r"(?:scored|rating of|base score of)\s+(\d+\.\d+)(?:\s+(?:on|in)\s+the\s+CVSS)",
            re.IGNORECASE,
        ),
    ...
    for pattern in cvss_patterns:
        for match in pattern.finditer(text):
            ...
            scores.append({"score": score, "version": version, "severity": severity})
...
def analyze_article_exploitation(article: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        "zero-day" in text.lower()
        or "0day" in text.lower()
        or "zero day" in text.lower()
    ):
        result["has_exploitation_content"] = True
